...
class Cost:
    ...
    # 均方误差
    def mse(self):
        return (self.cost_value / (self.samples_count * self.outputs_count) * 0.5) if self._can_calculate() else -1

    @property
    # 均方根误差
    def rmse(self):
        return np.sqrt(self.cost_value / (self.samples_count * self.outputs_count)) if self._can_calculate() else -1
    ...
    def cross_entropy(self):
        ...
        for index, sample_outputs in enumerate(self.outputs):
            ...
            sample_entropy = 0.0
            for net_index, net_output in enumerate(sample_outputs):
                target_value = sample_targets[index]
